...
void reverseRow(long arr[], int start, int end)
{
  while (start < end)
  {
    long temp = arr[start];
...
void swapRows(long arr[], int a, int offset)
{
  int b = 15 - a;
  for (int i=0; i<16; i++) {
    long temp = arr[offset + a + i];
    arr[offset + a + i] = arr[offset + b + i];
    arr[offset + b + i] = temp;
  }
}

void transposeImage(long image[]) {
  // // first reverse every other row
  // for(int i = 1; i <= 15; i += 2) {
  //   const int offset = 512 + (i*16);
  //   reverseRow(image, offset, offset+15);
  // }

  // // next, flip our last two images

  // for (int i = 0; i < 8; i++) {
  //    swapRows(image, i, 512);
  //    swapRows(image, i, 768);
  // }
}
